...
def draw():
    ...
    while (1):
        ...
        cv2.circle(finalImg, g_cen, 5, (0, 0, 255), -1)

        y_cen = drawCentroid(finalImg, y_area, yellowThreshImg, True)
        cv2.circle(finalImg, y_cen, 5, (0, 0, 255), -1)

        #print g_cen
        if g_cen!=None and y_cen!=None:
            if (g_cen[0] > 0 and g_cen[0] < 170 and g_cen[1] > 0 and g_cen[1] < 80) and (y_cen[0] > 0 and y_cen[0] < 170 and y_cen[1] > 0 and y_cen[1] < 80):
                 font = cv2.FONT_HERSHEY_SIMPLEX
